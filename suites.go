...
//
// The digest is checked before the bytes are parsed. The nearest thing in
// core-model.md §4 is step 5's boundary — the verified core object is what gets
// parsed — and the analogy stops there: §4 deliberately reads the protected
// header at step 3, before anything is authenticated, because a verifier must
// know which suite to verify with. So this is not "§4 forbids parsing before
// authentication"; it is a fail-closed choice available here and not there.
// Nothing in this file has to be read before the digest can be computed, so
// nothing is.
//
