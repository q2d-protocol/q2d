...
///
/// **This follows a decision [`is_q2d_timestamp`] already made; it does not make
/// one, and it is not in `spec/`.** §2.2 fixes the spelling and says nothing
/// about how an accepted `:60` participates in arithmetic, and P-004 §4.4
/// specifies the subtraction without saying either. A third implementation could
/// count it, collapse it, or refuse it, and disagree with both of these at a
/// leap boundary. That is [E-49](../docs/open-escalations.md), raised with the
/// other freshness constants that have no normative home, and this paragraph is
/// what stops the code being read as the answer.
///
