...
///
/// Ordering alone would be enough for *is this expired*, and `freshness.md` §2's
/// other two conditions are a window length and a skew tolerance. Those are
/// subtraction, so the conversion has to exist; making it the only primitive
/// means the ordering and the arithmetic cannot disagree. The values are that
/// document's and are deliberately not repeated here.
///
/// ## Leap seconds
///
...
///
/// **The rule is `freshness.md` §2.1**, which states it for every comparison and
/// subtraction the protocol makes. This is not the place it is decided: [E-49]
/// found that §2.2 fixes the timestamp spelling and said nothing about how an
/// accepted `:60` participates in arithmetic, so a third implementation could
/// have counted it, collapsed it, or refused it and disagreed with both of these
/// at a leap boundary. It closed by giving the rule a normative home, and this
/// function follows it rather than being it.
///
/// [E-49]: ../docs/open-escalations.md
///
