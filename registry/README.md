...
`validate.py` fixes that range at two through the registered cardinality, which
[`core-model.md`](../spec/core-model.md) §3.2 now states as well: condition 5
sets the floor at two labels and condition 3 the ceiling below the registered
cardinality ([`open-escalations.md`](../docs/open-escalations.md) E-27, closed).
A locally computed capacity is non-conforming
