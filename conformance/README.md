...
same answer on twenty-six documents — most chosen because a permissive parser
would differ about them, the rest because the contract itself must be read the
same way — and
the `rust and go` job runs it. Five were divergences when written — all about
