...
NETWORK_RUST = ("std::net", "tokio", "hyper", "reqwest", "ureq", "curl", "async-std", "surf")
...
def rust_uses(path: Path) -> set[str]:
    """Every path a `use` statement in this file names."""
    found = set()
    for line in path.read_text(encoding="utf-8").splitlines():
        stripped = line.strip()
        if stripped.startswith("use "):
            found.add(stripped[4:].rstrip(";"))
    return found
...
def go_imports(path: Path) -> set[str]:
    ...
    # Only inside an import block or a single-line import; a quoted string
    # elsewhere would be a false positive. Narrow by taking the block.
    block = re.search(r"^import \(\n(.*?)^\)", text, re.S | re.M)
    single = re.findall(r'^import (?:[\w.]+\s+)?"([^"]+)"', text, re.M)
    inside = set(re.findall(r'"([^"]+)"', block.group(1))) if block else set()
    return (inside | set(single)) & found
...
class RegistryClientTest(unittest.TestCase):
    ...
    def test_the_rust_registry_module_reaches_for_no_network(self):
        for used in rust_uses(REPO / "src" / "registry.rs"):
            for banned in NETWORK_RUST:
                self.assertNotIn(banned, used, f"src/registry.rs uses {used}")
...
class ImplementationTest(unittest.TestCase):
    ...
    def test_no_rust_source_file_reaches_for_the_network(self):
        ...
        for path in sorted((REPO / "src").rglob("*.rs")):
            for used in rust_uses(path):
                for banned in NETWORK_RUST:
                    self.assertNotIn(banned, used, f"{path.name} uses {used}")
            checked += 1
    ...
    def test_the_locked_crate_set_carries_no_network_client(self):
        # `Cargo.lock` rather than `Cargo.toml`, so a *transitive* dependency is
        # covered: the capability arrives whether or not this crate asked for it.
        locked = re.findall(r'^name = "([^"]+)"', (REPO / "Cargo.lock").read_text("utf-8"), re.M)
        self.assertIn("ed25519-dalek", locked, "the lock file did not parse as expected")
        for crate in locked:
            self.assertNotIn(crate, NETWORK_RUST, f"Cargo.lock carries {crate}")
...
class ReaderTest(unittest.TestCase):
    ...
    def test_the_rust_reader_finds_a_network_use(self):
        import tempfile

        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "fake.rs"
            path.write_text("use std::fmt;\nuse std::net::TcpStream;\n", encoding="utf-8")
            used = rust_uses(path)
            self.assertIn("std::net::TcpStream", used)
            self.assertTrue(any("std::net" in u for u in used))
