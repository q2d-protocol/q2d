...
| An explicit-escalation vector held to a denial's field list | It is not — §5.3's shape is `status: escalate`, `pending_token`, `expires_at`, receipt, signature, and it carries no `external_reason` because it is *"not denial-normalized and must never be described as such"*. An **opaque** escalation is held to §5.2's list, since §5.3 says it returns that same envelope and being indistinguishable is the whole point |
| A `denial/` vector asserting a projection rather than §5.2's whole response | `harness lint` rejects it. Enforced rather than documented: the two fields such a vector would carry are both fixed by the normalized class, so the comparison cannot fail, and a rule with nothing behind it is how a corpus comes to look like it verifies Q2D-C-08 |
| A corpus whose receipt timestamps mix RFC 3339 spellings — `T`/`t`, `Z`/`z`, or a numeric offset | `harness lint` fails it as a whole. No spelling alone is rejected — which §6 requires is open — but no implementation emits several, so none could satisfy such a corpus |
...
| **Which of RFC 3339's spellings may a receipt timestamp use?** [`core-model.md`](../../spec/core-model.md) §6 says `decided_at` is *"RFC 3339, second precision"* and does not say `Z`. But §6 also grounds the whole length guarantee in none of the reduced fields being variable-length, and `+00:00` is six characters where `Z` is one — so the same section appears to require `Z` by implication while not saying it. The same holds for `T` versus `t` and `Z` versus `z`, both of which RFC 3339 permits: three axes, one question. `harness lint` **allows any single spelling, names the one in use, and fails a corpus that mixes them** — which is defective whichever way §6 goes, since no implementation emits several, so none could satisfy such a corpus. Rejecting it would settle the question in a lint rule and push both implementations toward an uncited profile; ignoring it would let a corpus split across two forms go unnoticed until they disagree about which to emit. Deciding it either states `Z` in §6 or drops the length argument's dependence on this field | **Raised, awaiting decision.** A `core-model.md` §6 change. Small, and worth doing before signed receipts are authored, because every `denial/` vector will carry one |
