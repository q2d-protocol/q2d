...
| 11 | Fold `registry/` vectors into the corpus | **Done.** All fourteen manifest vectors run under `harness lint` and `harness run`, *generated* from the manifest by [`tools/fold_registry.py`](../../tools/fold_registry.py) rather than transcribed — the manifest outranks the corpus, so it stays the one place they live, and `--check` fails the build on any divergence. Five of them raised the step question in §10 |
