...
| 12 | Author `message/` section | Envelope, signing, verification, routing disagreement. **Unblocked.** [E-31](../open-escalations.md) settled the last of it — `eddsa-jws-2026`'s payload carries no `signature.value`, so the bytes are determined ([`crypto-suites.md`](../../spec/crypto-suites.md) §3). Before that: §10's header question is resolved and [`tools/author_vectors.py`](../../tools/author_vectors.py) assembles a signed string. Its response half — `sign_response`, `verify_response` — waits on **[E-32](../open-escalations.md)**, which asks what a signed response payload contains; so do `denial/` vectors, which assert a whole signed response |
