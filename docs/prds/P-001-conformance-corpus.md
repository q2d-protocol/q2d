...
| 13 | Author `suite/` section | **Done, less two groups that need a second registered suite or a new operation.** Nine vectors from [`tools/author_suite.py`](../../tools/author_suite.py): construction over a minimal payload, a valid verification, tampered payload and signature, and `downgrade/`'s four — an unregistered suite and an `alg`-carrying header at step 3, a header/payload suite mismatch and key mismatch at step 4, which is the ordering the suite's design rests on. `suite/keys/unresolvable` completes the authentication class. **`suite/rfc8032/` waits on issue 17** — signing a raw message needs an operation P-001 §4.5 does not have — and **`suite/status/` and the below-floor downgrade wait on a second registered suite**, since [`crypto-suites.md`](../../spec/crypto-suites.md) §3 registers one and it is `active` |
