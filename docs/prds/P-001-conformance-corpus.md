...
| 12 | Author `message/` section | Envelope, signing, verification, routing disagreement. **Unblocked** — §10's header question is resolved and [`tools/author_vectors.py`](../../tools/author_vectors.py) assembles a signed string. `denial/` vectors are a separate matter: §10's response-closure and timestamp-spelling questions decide bytes they would carry, so authoring them first would settle those by example |
| 13 | Author `suite/` section | Resolution, downgrade rejection, unknown suite. **Unblocked** — a downgrade vector asserts what a header declares, and the header is now specified. [P-003](P-003-crypto-suites.md) §6 gained two cases with it: a header carrying `alg` at all, and a header/payload key-id mismatch |
| 14 | Author `ordering/` section | One vector per rejection step 1–15. **Unblocked** — steps 4 and below need a signed envelope, which is now producible; steps 1–3 are pre-verification and need only the serializer |
