...
**No signature over a Q2D structure is committed yet.** §10 settled *how* one
gets authored — [`tools/author_vectors.py`](../../tools/author_vectors.py),
from the specification text rather than from an implementation — and building
that tool found what is still missing: the JWS protected header's member set,
which no document specifies. That is §10's remaining question, and the tool
...
| **At which step is a registry-entry constraint checked when it cannot be expressed in the entry's input schema?** [`core-model.md`](../../spec/core-model.md) §4 step 11 is *"public context validated against the entry's input schema"*. `availability-window` also carries a minimum slot duration, which no JSON Schema can express — so a rejection for it is a registry-entry constraint checked somewhere outside step 11's schema validation, and §4 does not say where. Both readings are defensible: broaden step 11 to *validated against the entry*, or give entry constraints a step of their own. Issue 11 folded those two vectors in **stating no step**, since §4.8 holds a vector only to the step it states, so nothing asserts the wrong thing meanwhile — but `before_private_access`, which the manifest vector does assert, survives only as prose in the description until this is settled | **Raised, awaiting decision.** A `core-model.md` §4 change, which CLAUDE.md makes an escalation on its own |
| **Does a receipt timestamp have to be `Z`, or may it carry a numeric offset?** [`core-model.md`](../../spec/core-model.md) §6 says `decided_at` is *"RFC 3339, second precision"* and does not say `Z`. But §6 also grounds the whole length guarantee in none of the reduced fields being variable-length, and `+00:00` is six characters where `Z` is one — so the same section appears to require `Z` by implication while not saying it. `harness lint` **accepts both and reports the offset form**, naming every vector that uses it. Rejecting it would settle the question in a lint rule and push both implementations toward an uncited profile; ignoring it would let a corpus split across two forms go unnoticed until they disagree about which to emit. Deciding it either states `Z` in §6 or drops the length argument's dependence on this field | **Raised, awaiting decision.** A `core-model.md` §6 change. Small, and worth doing before signed receipts are authored, because every `denial/` vector will carry one |
| **Is [`core-model.md`](../../spec/core-model.md) §5.2's deny response a closed field list, and where does retry metadata live if it is present?** §6 closes the receipt's list twice over — *"this is the authoritative field list"*, *"exactly five fields, and no others"* — and §5.2 says nothing equivalent about the response, while contemplating a field its own table does not list: *"if retry metadata is present, its value is identical across every cause mapped to that class"*. Meanwhile [P-009](P-009-denial-normalization.md) §4 and [P-013](P-013-https-binding.md) §4 forbid retry metadata on any Q2D outcome outright — *"no `429`, no `503`, no `Retry-After`"*. So either §5.2's sentence is about transport metadata that P-013 then bans, and the response list is effectively closed; or the response object has a retry field that is specified nowhere and never used. **`harness lint` therefore rejects a `denial/` vector missing a §5.2 field but not one carrying an extra**, which is the specification's asymmetry rather than a gap in the check — and the case that actually threatens Q2D-C-08, an extra field *differing* between causes, is caught by the uniformity assertion regardless. Deciding this closes the asymmetry one way or the other | **Raised, awaiting decision.** A `core-model.md` §5.2 change, and it touches [P-009](P-009-denial-normalization.md) and [P-013](P-013-https-binding.md) wherever they forbid retry metadata |
| **Which members does the JWS protected header carry?** [P-003](P-003-crypto-suites.md) §4.1 gives the structure — `BASE64URL(protected_header) "." BASE64URL(payload) "." BASE64URL(signature)` — and §4.2 says a verifier reads "the declared suite" from it. [`crypto-suites.md`](../../spec/crypto-suites.md) §3 implies an `alg` member by rejecting `alg: none`. **No document says which members the header actually carries.** `{"alg": "EdDSA", "kid": …}` follows JWS convention and makes `alg: none` a natural rejection; `{"suite": "eddsa-jws-2026", "kid": …}` names the Q2D suite directly, which is what §4.2 step 4 compares against the payload, and avoids a second identifier meaning almost the same thing. Both are defensible, which is what makes it an escalation rather than a choice. [P-002](P-002-message-envelope.md) §4.2's sorting rule fixes their *order* once the members are known, so **this is the last thing between a fully specified profile and a byte-exact signed vector**. `author_vectors.jws_compact()` raises rather than guessing — a guess would be a specification ambiguity resolved in a generator, and every signed vector in the corpus would then assert it | **Raised, awaiting decision.** A `spec/` or P-003 change. Blocks issues 12, 13, 14 and [P-003](P-003-crypto-suites.md)'s `suite/` vectors. **It also blocks one thing that is easy to lose:** `denial/` vectors must become whole-response — §5.2's four fields including the reduced receipt — and cannot, because a response carries a signature. Until then they assert a projection and the harness says so on every run. When this resolves, that upgrade rides with it, and `wire`'s subset rule tightens from prose into a schema requirement for `denial/` |
...
| 12 | Author `message/` section | Envelope, signing, verification, routing disagreement. **Blocked on the header question in §10** — every vector here carries a signature, and the tool that authors them refuses to assemble a JWS until the header's members are decided |
| 13 | Author `suite/` section | Resolution, downgrade rejection, unknown suite. **Blocked on the header question in §10** — a downgrade vector asserts what a header declares, so it cannot be authored before the header is specified |
| 14 | Author `ordering/` section | One vector per rejection step 1–15. **Partly blocked on the header question in §10**: steps 4 and below need a signed envelope; steps 1–3 are pre-verification and can be authored from the serializer alone |
