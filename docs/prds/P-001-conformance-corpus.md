...
| 17 | **Settle the §4.5 operation vocabulary for Stages 5–8, as one change** | Every operation named, with its owning PRD; no later PRD introduces one unilaterally. Closes after the endpoint drop and the requester-order addition, both already reflected in §4.5. **Now blocking [P-002](P-002-message-envelope.md) issue 10**, which cannot author `message/serialize/` or `message/envelope/` at all without `serialize_core`, `parse_core` and `parse_envelope`, and can only carry the part of `routing/` and `reject/` expressible through `verify_query` and `process_query`. P-002 issues 1–9 are built and their behaviour is asserted by mirrored unit tests, which catch a divergence only where the same case was written twice — so this is the issue standing between that work and a shared corpus |
