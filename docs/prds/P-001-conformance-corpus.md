...
| 13 | Author `suite/` section | **Ten vectors landed; three cases wait, none on this issue.** From [`tools/author_suite.py`](../../tools/author_suite.py): a valid verification, construction under a second key, a tampered payload, header and signature, an unresolvable key, an unregistered suite at step 3, and `downgrade/`'s three structural failures — an `alg`-carrying header at step 3, a header/payload suite and key mismatch at step 4, all three `structurally_invalid` after [E-34](../open-escalations.md). The two mismatch vectors assert **no step**: the comparison needs the parsed payload so it cannot precede §4 step 5, and §4's query order has no step for it — [E-35](../open-escalations.md), and a step-less vector asserts no ordering (§4.8) rather than a wrong one. **Waiting:** `suite/rfc8032/` on issue 17, since signing a raw message needs an operation §4.5 does not have; `suite/status/` and the below-floor downgrade on a second registered suite, where [`crypto-suites.md`](../../spec/crypto-suites.md) §3 registers one and it is `active`. Each absence is asserted in [`test_suite_section.py`](../../conformance/tests/test_suite_section.py) |
| 14 | Author `ordering/` section | One vector per rejection step 1–15, plus 9a and 11a. **Unblocked** — [E-33](../open-escalations.md) gave every step's rejection an `external_reason`: Tier A's five for steps 1, 3, 5, 6 and 8 — step 5 carries both `unsupported_version` and a `malformed` for a verified object missing a field §2 requires, since the authoritative values are inside the signed object — `unauthenticated` for 4 and 7, and the pinned registry's value from the replay check at 9, the rate limit at 9a, and 10 onward. Steps 4 and below need a signed envelope and steps 1–3 only the serializer, both of which exist |
