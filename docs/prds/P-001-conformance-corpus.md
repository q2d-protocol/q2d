...
| 13 | Author `suite/` section | **Seven vectors landed; five cases wait, none on this issue.** From [`tools/author_suite.py`](../../tools/author_suite.py): construction over a minimal payload, a valid verification, a tampered payload, header and signature, an unregistered suite rejected at step 3, before there is a signature to rely on, and an unresolvable key. **Waiting:** an `alg`-carrying header and the two header/payload mismatch cases on [E-34](../open-escalations.md), which asks what class a structurally invalid but authentic message produces; `suite/rfc8032/` on issue 17, since signing a raw message needs an operation §4.5 does not have; `suite/status/` and the below-floor downgrade on a second registered suite, where [`crypto-suites.md`](../../spec/crypto-suites.md) §3 registers one and it is `active`. Each absence is asserted in [`test_suite_section.py`](../../conformance/tests/test_suite_section.py) |
