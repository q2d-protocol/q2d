...
| 12 | Author `message/` section | Envelope, signing, verification, routing disagreement. **Blocked on [E-31](../open-escalations.md)** — every vector here asserts exact payload bytes, and whether `signature.value` is a payload field is unanswered. Everything else it needed is in place: §10's header question is resolved and [`tools/author_vectors.py`](../../tools/author_vectors.py) assembles a signed string. `denial/` vectors wait on the same answer, for the same reason |
| 13 | Author `suite/` section | Resolution, downgrade rejection, unknown suite. **Blocked on [E-31](../open-escalations.md)** — every vector here asserts exact payload bytes, and whether `signature.value` is a payload field is unanswered. Everything else it needed is in place: a downgrade vector asserts what a header declares, and the header is specified. [P-003](P-003-crypto-suites.md) §6 gained two cases with it — a header carrying `alg` at all, and a header/payload key-id mismatch |
| 14 | Author `ordering/` section | One vector per rejection step 1–15, plus 9a and 11a. **Blocked on [E-31](../open-escalations.md)** — every vector here asserts exact payload bytes, and whether `signature.value` is a payload field is unanswered. Everything else it needed is in place: steps 4 and below need a signed envelope and steps 1–3 need only the serializer, both of which exist |
