...
| 12 | Author `message/` section | Envelope, signing, verification, routing disagreement. **Query half unblocked**: [E-31](../open-escalations.md) settled the last of what it needed — a query payload carries no `signature.value`, so the bytes are determined ([`crypto-suites.md`](../../spec/crypto-suites.md) §3) — and before that, §10's header question was resolved and [`tools/author_vectors.py`](../../tools/author_vectors.py) assembles a signed string. **Response half blocked on [E-32](../open-escalations.md)**, which asks what a signed response payload contains; `sign_response` and `verify_response` vectors wait, and so do `denial/` vectors, which assert a whole signed response |
