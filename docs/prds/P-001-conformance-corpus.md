...
| 12 | Author `message/` section | Envelope, signing, verification, routing disagreement. **Blocked on [E-31](../open-escalations.md)** — every vector asserts exact payload bytes, and whether `signature.value` is one of them is unanswered. Otherwise unblocked — §10's header question is resolved and [`tools/author_vectors.py`](../../tools/author_vectors.py) assembles a signed string. `denial/` vectors are unblocked too: §10's timestamp-spelling question is resolved, so nothing left decides bytes they would carry **Blocked on [E-31](../open-escalations.md)** — every vector here asserts exact payload bytes, and whether `signature.value` is a payload field is unanswered |
| 13 | Author `suite/` section | Resolution, downgrade rejection, unknown suite. **Blocked on [E-31](../open-escalations.md)** — every vector asserts exact payload bytes, and whether `signature.value` is one of them is unanswered. Otherwise unblocked — a downgrade vector asserts what a header declares, and the header is now specified. [P-003](P-003-crypto-suites.md) §6 gained two cases with it: a header carrying `alg` at all, and a header/payload key-id mismatch **Blocked on [E-31](../open-escalations.md)** — every vector here asserts exact payload bytes, and whether `signature.value` is a payload field is unanswered |
| 14 | Author `ordering/` section | One vector per rejection step 1–15, plus 9a and 11a. **Blocked on [E-31](../open-escalations.md)** — every vector asserts exact payload bytes, and whether `signature.value` is one of them is unanswered. Otherwise unblocked — steps 4 and below need a signed envelope, which is now producible; steps 1–3 are pre-verification and need only the serializer **Blocked on [E-31](../open-escalations.md)** — every vector here asserts exact payload bytes, and whether `signature.value` is a payload field is unanswered |
