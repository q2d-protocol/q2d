...
| 13 | Author `suite/` section | **Sixteen vectors landed; two cases wait, neither on this issue.** From [`tools/author_suite.py`](../../tools/author_suite.py): a valid verification, construction under a second key, a tampered payload, header and signature, an unresolvable key, an unregistered suite at step 3, and `downgrade/`'s three structural failures — an `alg`-carrying header at step 3, and a header/payload suite and key mismatch, all three `structurally_invalid` after [E-34](../open-escalations.md). The two mismatch vectors assert step **5a**, which [E-35](../open-escalations.md) added to §4's query order for the comparison — it needs the parsed payload so it cannot precede step 5, and it precedes every step that acts on a payload field. **Waiting:** `suite/status/` and the below-floor downgrade, both on [E-48](../open-escalations.md) — a vector describes a *message*, and whether a suite is withdrawn or below the floor is a fact about the **verifier**. This row previously said they wait on a *second registered suite*, which was the wrong diagnosis: with one registered there would still be no way to write *this verifier lists it as withdrawn*. `suite/rfc8032/` is no longer waiting on this issue at all — P-003 §6 **retired** it rather than extend §4.5's vocabulary for a raw signature, since the known answers are already gated three ways. Each absence is asserted in [`test_suite_section.py`](../../conformance/tests/test_suite_section.py) |
