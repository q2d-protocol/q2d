...
| 12 | Author `message/` section | Envelope, signing, verification, routing disagreement. **Positive vectors landed** — `sign/query-minimal` asserting the compact string byte for byte, `verify/query-valid`, and `routing/subset`, generated by [`tools/author_message.py`](../../tools/author_message.py) with a `--check` in the suite so the committed bytes and the tool that produced them cannot drift. **Rejections blocked on [E-33](../open-escalations.md)**: a `routing`/`signed` disagreement is Tier A and an invalid signature is Tier B, and neither tier's classes have identifiers. Tier C's does — `unavailable`, which the registry declares — so `denial/uniformity-c/` is authorable and `denial/tier-a/` and `denial/uniformity-b/` are not |
| 13 | Author `suite/` section | Resolution, downgrade rejection, unknown suite. **Blocked on [E-33](../open-escalations.md)** — the section is rejections almost throughout, and an unacceptable or unknown suite is Tier A, whose classes have no identifier. Everything else is in place: [E-31](../open-escalations.md) determined the payload bytes, a downgrade vector asserts what a header declares, and [P-003](P-003-crypto-suites.md) §6 gained two cases with the header — `alg` present at all, and a header/payload key-id mismatch |
| 14 | Author `ordering/` section | One vector per rejection step 1–15, plus 9a and 11a. **Steps 10–15, 9a and 11a are authorable** — Tier C, whose class the registry declares as `unavailable`. **Steps 1–9 are blocked on [E-33](../open-escalations.md)**: they are Tiers A and B, whose classes have no identifier. Everything else is in place — [E-31](../open-escalations.md) determined the payload bytes, steps 4 and below need a signed envelope and steps 1–3 only the serializer, both of which exist |
