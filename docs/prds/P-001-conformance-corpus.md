...
| 14 | Author `ordering/` section | One vector per rejection step 1–15, plus 9a and 11a. **Unblocked** — [E-33](../open-escalations.md) gave every step's rejection an `external_reason`: Tier A's five for steps 1, 3, 5, 6 and 8 — step 5 is `unsupported_version`, since the authoritative `q2d_version` is inside the signed object — `unauthenticated` for 4 and 7, and the pinned registry's value from the replay check at 9, the rate limit at 9a, and 10 onward. Steps 4 and below need a signed envelope and steps 1–3 only the serializer, both of which exist |
