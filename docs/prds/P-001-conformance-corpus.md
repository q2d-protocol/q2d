...
| 12 | Author `message/` section | **Done.** Six vectors, generated by [`tools/author_message.py`](../../tools/author_message.py) with a `--check` in the suite: `sign/query-minimal` asserting the compact string byte for byte, `verify/query-valid`, `routing/subset`, and three rejections — `verify/wrong-signer`, `routing/disagrees`, `routing/introduces-field`. The rejections were blocked until [E-33](../open-escalations.md) enumerated the `external_reason` vocabulary in [`core-model.md`](../../spec/core-model.md) §5.2.1. No vector cites a claim: Q2D-C-05's *Verified by* names three vectors this section does not yet contain |
| 13 | Author `suite/` section | **Ten vectors landed; three cases wait, none on this issue.** From [`tools/author_suite.py`](../../tools/author_suite.py): a valid verification, construction under a second key, a tampered payload, header and signature, an unresolvable key, an unregistered suite at step 3, and `downgrade/`'s three structural failures — an `alg`-carrying header at step 3, and a header/payload suite and key mismatch, all three `structurally_invalid` after [E-34](../open-escalations.md). The two mismatch vectors assert step **5a**, which [E-35](../open-escalations.md) added to §4's query order for the comparison — it needs the parsed payload so it cannot precede step 5, and it precedes every step that acts on a payload field. **Waiting:** `suite/rfc8032/` on issue 17, since signing a raw message needs an operation §4.5 does not have; `suite/status/` and the below-floor downgrade on a second registered suite, where [`crypto-suites.md`](../../spec/crypto-suites.md) §3 registers one and it is `active`. Each absence is asserted in [`test_suite_section.py`](../../conformance/tests/test_suite_section.py) |
| 14 | Author `ordering/` section | **Twelve vectors landed; five steps wait on fixture formats other PRDs own.** From [`tools/author_ordering.py`](../../tools/author_ordering.py), every one using `process_query` — ordering is a property of the pipeline, and a `verify_query` vector cannot show that the signature was checked *before* the registry was consulted. Steps 1, 3, 4, 5, 5a, 6, 8 need only a crafted envelope; 10, 11, 11a, 12 and 13 need the pinned registry, which a runner embeds. Each carries the responder's clock as `environment.now` — §4 step 6 is the first thing in the pipeline that needs a time, and P-001 §4.3 forbids a runner from reading one. **Waiting on a fixture format:** 7 (a delegation profile, [P-014](P-014-identity-pairing.md)), 9 and 9a ([P-004](P-004-replay-idempotency.md)), 14 ([P-007](P-007-policy-engine.md)), 15 ([P-008](P-008-capacity-accounting.md)). **Step 2 gets none by design** — §4 makes it optional and never a security decision, so a responder that sheds there and one that does not are both conforming. [P-010](P-010-responder-pipeline.md) §6 owns the section and its issue 11 completes it; this authors what needs no fixture, so the ordering exists before Stage 4 rather than after |
