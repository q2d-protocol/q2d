...
| 13 | Author `suite/` section | Resolution, downgrade rejection, unknown suite. **Unblocked** — [E-33](../open-escalations.md) gave an unregistered or unacceptable suite its `external_reason`, `unsupported_suite`, which is one value for both causes so a requester cannot learn the custodian's floor. [E-31](../open-escalations.md) determined the payload bytes, and [P-003](P-003-crypto-suites.md) §6 has two more cases the header made assertable — `alg` present at all, and a header/payload key-id mismatch |
