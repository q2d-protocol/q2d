...
| 12 | Author `message/` section | Envelope, signing, verification, routing disagreement. **Positive vectors landed** — `sign/query-minimal` asserting the compact string byte for byte, `verify/query-valid`, and `routing/subset`, generated by [`tools/author_message.py`](../../tools/author_message.py) with a `--check` in the suite so the committed bytes and the tool that produced them cannot drift. **Rejections blocked on [E-33](../open-escalations.md)**: every one asserts the `external_reason` a requester receives, and no normalized class has an identifier. `denial/` waits on the same answer |
| 13 | Author `suite/` section | Resolution, downgrade rejection, unknown suite. **Blocked on [E-33](../open-escalations.md)** — the section is rejections almost throughout, and each must assert the `external_reason` a requester receives, which no normalized class has. Everything else is in place: [E-31](../open-escalations.md) determined the payload bytes, a downgrade vector asserts what a header declares, and [P-003](P-003-crypto-suites.md) §6 gained two cases with the header — `alg` present at all, and a header/payload key-id mismatch |
