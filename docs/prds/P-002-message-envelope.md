...
| 10 | Author `message/` corpus section | All five groups present; `harness lint` clean **Authored through the operations that already exist**, and this is worth stating because the first reading of the `operation` enum is that four of these five groups need names it does not have. They do not. The corpus tests protocol behaviour through protocol operations, so §4.2's serializer is exercised by a `sign_query` vector asserting the compact string byte for byte — which is exactly what `message/sign/query-minimal` already does — and the parser and §2.8's limits by `verify_query` and `process_query` vectors whose payloads are malformed in one way each. `digest` is in the enum on its own account.
