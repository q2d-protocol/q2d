...
| **Does §2.2's timestamp spelling bind every string, or only the fields §2.2 names?** | **[E-36](../open-escalations.md), open.** §4.2 cites `core-model.md` §2.2 for the spelling; neither states the reach. `tools/author_vectors.py` refuses *any* string with an RFC 3339 spelling that is not §2.2's, at any depth — including inside `public_context`, which §2.6 says is operation-defined — and issue 2 copied that into both implementations rather than let the serializer disagree with the tool that generates the corpus's expected bytes. So the rule is narrower in `spec/` than in the code, which is a resolution living in a tool. Three implementations agree today, so nothing is divergent; the register has the options and my recommendation |
