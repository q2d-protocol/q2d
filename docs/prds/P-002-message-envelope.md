...
**Nothing in `serialize/`, `envelope/`, `digest/` or `reject/` is written.** `conformance/corpus/message/` holds `sign/`, `verify/` and three `routing/` vectors, all from P-001 issue 12. Until the rest exist, every behaviour listed below is covered only by two suites written to mirror each other.
