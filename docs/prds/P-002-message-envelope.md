...
| 10 | Author `message/` corpus section | All five groups present; `harness lint` clean **Blocked on [P-001](P-001-conformance-corpus.md) issue 17 for four of its five groups.** `conformance/vector.schema.json`'s `operation` enum is closed and says so: *"Stages 5–8 extend this list, as one coordinated change, under P-001 issue 17 — the names anticipated there are proposals, not decisions, so they are deliberately absent here."* It has no `serialize_core`, `parse_core`, `parse_envelope`, `project_routing` or `check_routing`, so `serialize/` and `envelope/` cannot be authored at all, and `routing/` and `reject/` can only carry the cases expressible through `verify_query` and `process_query` — which is how P-001 issue 12's three routing vectors were written. **`digest/` is the exception**: `digest` is in the enum, though no vector uses it yet, so its input shape would be settled by the first one. That ordering is worth respecting rather than working around: issue 17 exists because an operation introduced by one PRD in passing is one no other PRD agreed to.
