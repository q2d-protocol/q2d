...
| **May an envelope omit `routing`?** | **[E-38](../open-escalations.md), open.** §2.1 opens *"a message has two parts"* and calls `routing` advisory, which answers what it may be *used for* rather than whether it may be *absent*. The corpus already contains an envelope without one — `suite/downgrade/header-carries-alg.json`, authored under [P-001](P-001-conformance-corpus.md) issue 13 and merged — so requiring it would retroactively make landed work malformed. **Both implementations require it**, which is what §2.1 says. I implemented the permissive reading first, on the argument that the corpus's authored vector was evidence of intent — and CLAUDE.md's hierarchy answers that directly: `spec/` outranks the corpus, a PRD and a tool, so a practice built on an unstated reading does not get to override §2.1. The ten `suite/` vectors now carry a projection. My view is that `routing` is an optimisation for a party a direct exchange does not have, and requiring it publishes `predicate.id` and `target.custodian` in the clear for nobody's benefit |
