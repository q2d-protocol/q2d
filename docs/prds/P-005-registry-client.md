...
| Blocks | P-006, P-007, P-010, P-011, P-016 — ~~P-008, P-012~~ **deferred 2026-08-19** |
...
| 9 | Dependency assertion: no network client in this module | **Done, in both** — [`conformance/tests/test_no_network.py`](../../conformance/tests/test_no_network.py), in CI with the rest of the suite. **Two kinds of check, and the test says which is which.** *Absence is established* by the linked sets: an **allowlist over `Cargo.lock`** for Rust, so nothing outside the exact locked set is linked, and a **denylist over `go list -deps ./...`** for Go — the transitive package closure actually linked, so a networking package absent from it is absent from the binary. A denylist is complete over that closure because the set is the real one; an allowlist over ~112 mostly-`internal/` packages would break on every toolchain upgrade and be updated without being read. *Absence is **not** established* by the source scans, which are a **tripwire** for the ordinary spellings. Eight review rounds each found another legal way past a text scan — brace-grouped `use`, fully-qualified calls, raw-string import paths, `use std::{net as network}` — and a macro or an FFI block defeats the approach outright. They catch what an accident looks like, the same honest limit [P-002](P-002-message-envelope.md) §8 states about `Routing`. **Two claims review corrected**, and both were mine: a `Cargo.lock` *denylist* cannot support a statement about transitive dependencies, and Go's half is not unachievable — always-*available* is not always-*linked*. **It will need scoping when [P-013](P-013-https-binding.md) lands**, since the binding links a server; the right change then is to narrow these to the registry's own files rather than delete them, which the test says at its top |
