...
| 6 | Fail-to-serve state | **Half done, and the half that is left is not this module's.** There is no `Manifest` value except one that parsed and whose every entry digest recomputed, so a responder that never loaded has nothing to resolve against — `resolve` is a method on the thing it does not have, which is the same shape as `SuitePolicy`. **That is not the whole of §4.2.** The rule that a *later* load failure must not leave a responder serving from the manifest it already has is a property of whatever holds one across a reload, and nothing here reloads: this module has no serving state to go stale. Review found the row claiming otherwise. It belongs with [P-013](P-013-https-binding.md) §4.6's startup, where the daemon either starts with a verified manifest or does not start |
...
| 9 | Dependency assertion: no network client in this module | **Done, in both.** Rust by an **allowlist over `Cargo.lock`** — nothing outside the exact locked set is linked. Go by a denylist over **`go list -deps ./...`**, the transitive package closure actually linked, so a networking package absent from it is absent from the binary. A denylist is complete there because the set is the real closure; an allowlist over ~112 mostly-`internal/` packages would break on every toolchain upgrade and be updated without being read. **An earlier version of this row said Go's half was unachievable** because networking is in the standard library — review corrected it, and always-*available* is not always-*linked*. [`conformance/tests/test_no_network.py`](../../conformance/tests/test_no_network.py), in CI with the rest of the suite. **Two halves, worth different amounts, and the test says which.** The **allowlist over `Cargo.lock`** establishes absence: nothing outside the exact locked set is linked, and a network crate cannot arrive without changing that list. The **source sweeps are a tripwire**, not a proof — six review rounds each found another legal spelling the text scan missed, ending on `use std::{net as network}`, and a macro or an FFI block defeats the approach entirely. They catch what an accident looks like, which is the same honest limit [P-002](P-002-message-envelope.md) §8 states about `Routing`. **Go has no allowlist half at all**, since its networking is in the standard library and always linked; for Go the tripwire is all there is, and the test says so rather than implying otherwise. The Rust crate set is checked as an **allowlist over `Cargo.lock`**, which is what lets the claim be about *transitive* dependencies at all — review found the first version using a denylist, which covers only the crates someone thought of. Its cost is that the list changes when `ed25519-dalek`'s tree does, and that is the feature: a new transitive dependency, in a repository whose stated posture is one dependency, should be a decision somebody makes rather than a lock-file diff nobody reads. The `go.mod` half is **weaker by construction and says so**: Go's standard library is always available, so a module list can never show `net/http`, and the per-file sweep is what covers that. Matching is prefix-on-a-path-boundary, so `golang.org/x/net/http2` is caught and a module called `magnet` is not. **It will need scoping when [P-013](P-013-https-binding.md) lands** — the binding links a server, and the right change then is to narrow these to the registry's own files rather than delete them, which the test says at the top rather than leaving to be inferred from a failure |
...
| 12 | Recompute every `entry_digest` at load | **Done**, and it catches **one** of two things — review corrected a test name that claimed both. Recomputation refuses an entry whose stored digest has gone **stale**: an edit nobody re-digested, or a splice into a manifest. It does **not** refuse a publisher who edits an entry *and* updates its digest, since that manifest is self-consistent and recomputation agrees with it — what refuses that is §4.1's **pin** over the whole manifest, which a custodian changes only after reading the diff. At parse time while the entry's parsed value is still in hand — recomputing later would mean keeping every entry's raw value beside its typed form, which is two representations that can disagree. `entry_digest` is **removed** rather than blanked, since a digest over an entry carrying an empty one would be a different rule and still self-referential in shape. **The reference manifest's three stored digests were authored by [`registry/validate.py`](../../registry/validate.py) in Python and now recompute in Rust and in Go** — three readings of `entry_digest_rule` that have to agree, which is what checks the rule rather than any fixture |
