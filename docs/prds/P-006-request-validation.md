...
| **Is a release that cannot vary with the data admissible?** §3.2's four conditions admit a coarsening onto a *single* label; `registry/validate.py` and §3.2's `boolean` rationale both say no. §2.5 permits an empty `allowed_detail_fields`, which is the same thing for an `object`, and one answer has to cover both. | **Open — E-27** ([`open-escalations.md`](../open-escalations.md)). **Blocks issue 4** for two shapes: `check_narrowing` is the `enum` condition list, so building it settles whether there are four conditions or five, and it is also what decides whether an `object` request with an empty `allowed_detail_fields` is admissible. Both are the same zero-debit release path. Every other shape is unaffected and may proceed |
...
Two of its shapes are now **blocked on E-27**, for the same reason E-17 existed.
`check_narrowing` *is* the `enum` condition list, so writing it decides whether
there are four conditions or five; and it is what decides whether an `object`
request with an empty `allowed_detail_fields` is admissible, which §2.5 permits
as a value while saying nothing about what it means. Both are the same question —
may a requester ask for an answer that cannot vary with the data, at a debit of
zero — and building either would settle in code a disagreement
`registry/validate.py` and §3.2 currently have with each other. Every other shape
is unaffected.
