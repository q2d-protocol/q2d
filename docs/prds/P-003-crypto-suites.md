...
| `suite/status/` | Deprecated verifies but will not produce; withdrawn refuses both. Both vectors carry a registry of their own under `input.verifier` — the status is the verifier's, not the message's ([E-48](../open-escalations.md)) |
...
| 6 | `verify` — the four-step sequence in §4.2 | **Done** — [E-46](../open-escalations.md) closed as B and unblocked it, and the tests build each rejection from a **validly signed** message so that each is about the sequence rather than the mathematics: a header carrying `alg`, an **unregistered** suite, and a header naming a different key all carry signatures that verify and are all refused. The rest of step 2 followed later and separately: a suite **below the policy floor**, and one the registry has **withdrawn**, are facts about the verifier rather than about the message, so neither had a vector until [E-48](../open-escalations.md) gave a vector a way to describe the verifier. Both do now, and the step reads status before policy so the two are not reported as one. `verify_compact` splits the compact string, checks all three segments are base64url, and verifies the signature over the received text of the first two — it is **crate-internal in Rust and unexported in Go**, deliberately: it does not read the suite, consult the policy, compare header against payload, or check the header is §3's closed two-member object, so a public function returning *verified payload bytes* from a string whose header was never inspected would invite a caller to treat those bytes as a Q2D message. They are bytes with a good signature over them, which is weaker. **It stays that way**: `verify_query` is the public surface now and `verify_compact` is the piece beneath it, `#[cfg(test)]` in Rust and unexported in Go, because a caller wanting a verified message wants the sequence rather than its cryptographic half. Step 5 reports the five internal reasons `message/reject/` keeps apart — `core_object_duplicate_key`, `core_object_float`, `core_object_too_deep`, `core_object_too_many_members`, `core_object_string_too_long` — which needed `parse` to carry a **typed cause** beside its message in both languages. Recovering them by matching prose would have been a second place deciding what a parse failure was. Review raised the gap twice and was right both times. **A second gap is named at the constant in both**: `core_object_missing_required_field` is not produced, because checking that every field §2 requires is present is core-object validation rather than suite verification — [P-005](P-005-registry-client.md)'s, and the vector belongs to whoever builds it |
...
| 8 | Status enforcement, both directions | **Done.** The producing half is enforced where it cannot be forgotten: `sign` takes a `SuiteEntry` and refuses unless its status permits production, so there is no path that signs under a deprecated or withdrawn suite by naming it. The verifying half is now in two places rather than one, and the second was found while closing [E-48](../open-escalations.md): `policy`'s floor refuses a **configured** withdrawn suite at startup, and step 2 refuses a **declared** one at verification. Those are different messages — the first is an operator's config file, the second is a suite the deployment never configured and the registry has retired — and collapsing them left `suite_below_policy` reporting a registry decision as a local one. Both implementations report `suite_withdrawn` for it, and step 2 reads **status before policy** so the refusal is attributed to whoever made it. `suite/status/deprecated-verifies` and `suite/status/withdrawn-refuses` are the vectors |
...
| 11 | Author `suite/` corpus section | **Done — all five of §6's groups, nineteen vectors.** `suite/rfc8032/` is **retired** rather than owed: a raw Ed25519 signature is not a Q2D operation, and retiring the row was the alternative to extending [P-001](P-001-conformance-corpus.md) §4.5's vocabulary for it — the right trade, because the known answers are already gated three ways and a corpus group would have re-asserted what three tests assert. `suite/verify/` also gained `payload-not-base64url` and `header-not-an-object`, so every structural rejection the sequence can reach has a shared vector — review found the two that did not, which is the gap a cross-implementation claim is made of. **`suite/status/` and the below-floor case landed with [E-48](../open-escalations.md)**, which this row raised: they needed something the vector format did not have, a way to state the *verifier's* registry and acceptable set, since a deprecated or withdrawn suite is a fact about the verifier rather than about the message. It closed as `input.verifier` — [P-001](P-001-conformance-corpus.md) §4.3 — and all five groups are complete at nineteen vectors. The three suite refusals are asserted **together**, because each passes alone while their `external_reason`s diverge — the mapping, not indistinguishability, which these vectors project too few fields to show |
