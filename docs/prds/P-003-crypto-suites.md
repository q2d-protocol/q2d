...
| 6 | `verify` — the four-step sequence in §4.2 | **Done** — [E-46](../open-escalations.md) closed as B and unblocked it, and the tests build each rejection from a **validly signed** message so that each is about the sequence rather than the mathematics: a header carrying `alg`, a suite outside the acceptable set, a header naming a different key all carry signatures that verify and are all refused. `verify_compact` splits the compact string, checks all three segments are base64url, and verifies the signature over the received text of the first two — it is **crate-internal in Rust and unexported in Go**, deliberately: it does not read the suite, consult the policy, compare header against payload, or check the header is §3's closed two-member object, so a public function returning *verified payload bytes* from a string whose header was never inspected would invite a caller to treat those bytes as a Q2D message. They are bytes with a good signature over them, which is weaker. It becomes public when this row is done. **One gap is owed and recorded at the line where it lives in both modules**: every parse failure at step 5 collapses to one internal reason, where `message/reject/` distinguishes five — `core_object_duplicate_key`, `core_object_float`, `core_object_too_deep`, `core_object_too_many_members`, `core_object_string_too_long`. The wire value is right for all of them and the internal reason is not, so closing it means `parse` reporting a **typed cause** rather than a message string, which is [P-002](P-002-message-envelope.md)'s error surface in both languages rather than this module's. **A second gap is named at the constant in both**: `core_object_missing_required_field` is not produced, because checking that every field §2 requires is present is core-object validation rather than suite verification — [P-005](P-005-registry-client.md)'s, and the vector belongs to whoever builds it |
